...
try {
    ...
    // patients table
    // `mrn` / `phone` / `national_id` / `email` carry reception (MedCore)
    // identity so consultations sent from reception match an existing patient
    // instead of cloning one.  `national_id` mirrors medcore_hms.patients.national_id
    // and is used as a secondary deduplication key by the bridge.
    ...
    // carries the reason for visit captured at reception; `duration` mirrors the
    // slot length (minutes) from medcore_hms.appointments.duration.
    ...
    // consultations table — full clinical case sheet record.
    // `nursing_plan` stores the structured nursing JSON blob (pre/intra/post procedure).
    // `ros_data` / `exam_data` store Review of Systems and Clinical Examination JSON.
    // `created_at` / `updated_at` provide an audit trail for draft → finalized edits.
    ...
    foreach ($seedData as $data) {
        ...
        $insertAppt = $pdo->prepare("INSERT INTO `appointments` (`patient_id`, `doctor_id`, `appointment_date`, `appointment_time`, `status`, `duration`) VALUES (?, ?, ?, ?, 'Scheduled', 30)");
